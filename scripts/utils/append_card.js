...
const appendCard = (card, to, cssClass = "") => {
    ...
    const createAttributeDom = (attribute, label, singularText, pluralText) => {
    ...
    const ageDom = createAttributeDom("age", "Age", "Card has been retained for 1 turn this combat", "Card has been retained for " + age + " turns this combat");
    const useDom = createAttributeDom("use", "Use", "Number of times this card can be used before vanishing", "");
    const expireDom = createAttributeDom("expire", "Expire", "Number of turns this card remains in your deck before vanishing", "");
    const lingerDom = createAttributeDom("linger", "Linger", "Number of times this card can be played before it leaves your hand", "");

    const createBooleanAttributeDom = (attribute, label, description) => {
        const value = getCardAttribute(card, attribute);
        if (value) {
    ...
    const vanishDom = createBooleanAttributeDom("vanish", "Vanish", "Becomes a dead card when played");
    const retainDom = createBooleanAttributeDom("retain", "Retain", "Does not discard at the end of your turn");
    const ephemeralDom = createBooleanAttributeDom("ephemeral", "Ephemeral", "Becomes a dead card if drawn but not played");
    const breakableDom = createBooleanAttributeDom("breakable", "Breakable", "When used up, card is permanently removed from your deck");
    ...
    const naturalDom = createBooleanAttributeDom("natural", "Natural", "This card starts on the top of your draw pile each combat");
    const auraDom = createBooleanAttributeDom("aura", "Aura", "This card starts in your hand each combat");
    const sparkleDom = createBooleanAttributeDom("sparkle", "Sparkle", "This card starts in your hand each combat");
    const shimmerDom = createBooleanAttributeDom("shimmer", "Shimmer", "This card starts in your hand each combat");
    const tradeDom = createBooleanAttributeDom("trade", "Trade", "This card can be traded away at the end of each combat");
    const weaponDom = createBooleanAttributeDom("weapon", "Weapon", "This card can be used to attack enemies in combat");
    const aoeDom = createBooleanAttributeDom("aoe", "AoE", "This card can be used to attack all enemies in combat");
    ...
    $(`<div class='card-wrapper drawing'>
        <div class='card ${card.tier}${unplayable}${combinable}${pack} ${card.type} ${cssClass}' id='card-${card.id}' data-id='${card.id}' data-guid='${card.guid}' data-powertip='${tooltip}'>
            ...
            <div class='bubbles-right'>${vanishDom}${retainDom}${ephemeralDom}${breakableDom}${combinableDom}${auraDom}${sparkleDom}${shimmerDom}${tradeDom}${weaponDom}${aoeDom}${unaddable}${naturalDom}</div>
